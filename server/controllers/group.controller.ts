...
export const listHandler = async (req: Request, res: Response) => {
  try {
    const groups = await listGroups(getWorkspaceId(req), typeof req.query.search === 'string' ? req.query.search : undefined);
    res.json(groups);
  } catch (err) {
...
export const createHandler = async (req: Request, res: Response) => {
  try {
    const group = await createGroup(getWorkspaceId(req), getUserId(req), req.body);
...
export const renameHandler = async (req: Request, res: Response) => {
  try {
    const group = await renameGroup(getWorkspaceId(req), req.params.id, req.body);
    res.json(group);
  } catch (err) {
...
export const deleteHandler = async (req: Request, res: Response) => {
  try {
    await deleteGroup(getWorkspaceId(req), req.params.id);
    res.status(204).send();
  } catch (err) {
...
export const assignContactsHandler = async (req: Request, res: Response) => {
  try {
    const result = await assignContacts(getWorkspaceId(req), req.params.id, req.body?.contactIds);
    res.json(result);
  } catch (err) {
...
export const removeContactsHandler = async (req: Request, res: Response) => {
  try {
    const result = await removeContacts(getWorkspaceId(req), req.params.id, req.body?.contactIds);
    res.json(result);
  } catch (err) {
